...
/**
 * Validates `config.baseUrl` (if provided) and returns the resolved base URL.
 * Throws if the URL is malformed or uses an insecure scheme in a non-local context.
 */
export function resolveBaseUrl(
    config: Pick<GoPayConfig, 'baseUrl' | 'environment'>,
): string {
    const rawBaseUrl =
